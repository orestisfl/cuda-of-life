...
__global__ void calculate_next_generation(const bboard* d_a,
  ...
  const int major_j = __mul24(blockIdx.y, blockDim.y) + tx;  // col
  const int major_i = __mul24(blockIdx.x, blockDim.x) + ty;  // row
  ...
  tiles[TPOS(tx + 1, ty + 1, by)] = (d_a + major_i * pitch)[major_j];

  if (ty == 0) {
    //is in the left edge of the block. keep row
    tiles[TPOS(tx + 1, 0, by)] = (d_a + major_i * pitch)[major_l];

    if (tx == 0) {
      //top left corner!
      tiles[TPOS(0, 0, by)] = (d_a + major_t * pitch)[major_l];
    }

    if (tx == bx - 3) {
      //bottom left corner
      tiles[TPOS(bx - 1, 0, by)] = (d_a + major_b * pitch)[major_l];
    }
  }

  if (ty == by - 3) {
    //is on the right edge
    tiles[TPOS(tx + 1, by - 1, by)] = (d_a + major_i * pitch)[major_r];

    if (tx == 0) {
      // top right corner
      tiles[TPOS(0, by - 1, by)] = (d_a + major_t * pitch)[major_r];
    }

    if (tx == bx - 3) {
      // bottom right corner
      tiles[TPOS(bx - 1, by - 1, by)] = (d_a + major_b * pitch)[major_r];
    }
  }

  if (tx == 0) {
    //is on the upper edge of the block. keep col
    tiles[TPOS(0, ty + 1, by)] = (d_a + major_t * pitch)[major_j];
  }

  if (tx == bx - 3) {
    //is on the bottom edge
    tiles[TPOS(bx - 1, ty + 1, by)] = (d_a + major_b * pitch)[major_j];
  }
